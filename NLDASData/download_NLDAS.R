...
extent = c(-72.0625,43.1875,-72.032850,43.2328) #coords taken from NKW
#if the extent is loaded from the shapefile (above), make sure they are in decimal degrees, otherwise this code will not work

###########################################################
### Set timeframe
###########################################################
startdatetime = '2018-12-31 06:00:00'
enddatetime = '2019-01-01 07:00:00'
loc_tz = 'UTC'
...
for (i in 1:length(out.ts)) {
  ...
  resp <- curl::curl_fetch_disk(url = lk, 
                                path = paste(dumpdir_nc, '/NLDAS_Data_2019_2020/', filename, '_', loc_tz, '.nc',sep=''), 
                                handle = h)
