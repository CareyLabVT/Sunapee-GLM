...
#*                                                               
#* TITLE:   GLM_overlandflow_model                               
#* AUTHOR:  B. Steele                                            
#* SYSTEM:  Lenovo ThinkCentre 800, Win 10, R 4.0.3              
#* DATE:    09Feb2021                                            
#* PROJECT: CIBR/FLARE                                           
#* PURPOSE: generalize the inflow modeled on overland flow from  
#*          Sunapee based on NLDAS-2 met data and GIS analysis   
#*          of impervious surfaces, as calculated in Ward et al. 
#*          2020                                                 
#* LAST MODIFIED: 09Feb2021                                      
#* BY:      B. Steele                                            
#* NOTES:   based on the code 'CNH-GLM_sunapee_inflowoutflow_23Mar2017.R
#*****************************************************************
...
#MetData <- read.csv('SunapeeMet_1979_2016.csv', header=T, na.strings=NA) #in begwrkspc
...
write.csv(impLUpro_c, 'Sunapee WS impervious land cover summary 15Mar2017.csv', row.names = F)
write.csv(LU_all, 'Sunapee WS land cover summary 15Mar2017.csv', row.names=F)
...
MetData$date <- as.Date(MetData$dateTime)
...
#merge metdata daily file with temp daily file
run505_temp <- merge(MetData_505_daily_sub, temp_daily, by='date', all.x=T)
run510_temp <- merge(MetData_510_daily_sub, temp_daily, by='date', all.x=T)
run540_temp <- merge(MetData_540_daily_sub, temp_daily, by='date', all.x=T)
run665_temp <- merge(MetData_665_daily_sub, temp_daily, by='date', all.x=T)
run760_temp <- merge(MetData_760_daily_sub, temp_daily, by='date', all.x=T)
run788_temp <- merge(MetData_788_daily_sub, temp_daily, by='date', all.x=T)
run790_temp <- merge(MetData_790_daily_sub, temp_daily, by='date', all.x=T)
run800_temp <- merge(MetData_800_daily_sub, temp_daily, by='date', all.x=T)
run805_temp <- merge(MetData_805_daily_sub, temp_daily, by='date', all.x=T)
run830_temp <- merge(MetData_830_daily_sub, temp_daily, by='date', all.x=T)
run835_temp <- merge(MetData_835_daily_sub, temp_daily, by='date', all.x=T)
runung_temp <- merge(MetData_ung_daily_sub, temp_daily, by='date', all.x=T)
...
range(inf_base$model_imp_50_50_plusbase_m3ps)
...
write.csv(inf_base_505_s, file='505_totalinflow_temp_23Mar2017.csv', row.names = F)
...
write.csv(inf_base_510_s, file='510_totalinflow_temp_23Mar2017.csv', row.names = F)
...
write.csv(inf_base_540_s, file='540_totalinflow_temp_23Mar2017.csv', row.names = F)
...
write.csv(inf_base_665_s, file='665_totalinflow_temp_23Mar2017.csv', row.names = F)
...
write.csv(inf_base_760_s, file='760_totalinflow_temp_23Mar2017.csv', row.names = F)
...
write.csv(inf_base_788_s, file='788_totalinflow_temp_23Mar2017.csv', row.names = F)
...
write.csv(inf_base_790_s, file='790_totalinflow_temp_23Mar2017.csv', row.names = F)
...
write.csv(inf_base_800_s, file='800_totalinflow_temp_23Mar2017.csv', row.names = F)
...
write.csv(inf_base_805_s, file='805_totalinflow_temp_23Mar2017.csv', row.names = F)
...
write.csv(inf_base_830_s, file='830_totalinflow_temp_23Mar2017.csv', row.names = F)
...
write.csv(inf_base_835_s, file='835_totalinflow_temp_23Mar2017.csv', row.names = F)
...
write.csv(inf_base_ung_s, file='ung_totalinflow_temp_23Mar2017.csv', row.names = F)
...
#write .csv
setwd("C:/Users/steeleb/Dropbox/Lake Sunapee/Lake Sunapee Lake Model/Final Data/inflow plus baseflow")
write.csv(balance_inflow_sub, 'balance_inflow_temp_23Mar2017.csv', row.names = F)
...
write.csv(corr_outflow_sub, 'corr_outflow_impmodel_baseflow_23Mar2017.csv', row.names = F)
