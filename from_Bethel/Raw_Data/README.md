...
# Inflow
*inflowchem_aggregatedonevalperday_01Mar2021* <- all stream chemistry data for sources available 1986-2020. 
	Data have been aggregated to a mean value where multiple values per parameter per stream
	per day were recorded. Data provenance varies. (Replaces 'inflowchem_onevalperday_21Sept2020.csv')
	...
	* DOC_mmolm3: dissolved organic carbon in millimols per meter cubed	(mean if DOC_rep >= 2)
	* DOC_flag: lists pertinent flags, 'suspect' observations should be handled with care	
	* DOC_source: lists the source(s) of the data
	* TN_rep: number of values aggregated to mean in 'TN_mmolm3'	
	* TN_mmolm3: total nitrogen in millimols per meter cubed (mean if TN_rep >= 2)
	* TN_flag: lists pertinent flags, 'suspect' observations should be handled with care	
	* TN_source: lists the source(s) of the data
	* TP_rep: number of values aggregated to mean in 'TP_mmolm3'	
	* TP_mmolm3: total phosphorus in millimols per miter cubed (mean if TP_rep >= 2)
	* TP_flag: lists pertinent flags, 'suspect' observations should be handled with care
	* TP_source: lists the source(s) of the data

*inflowchem_TNTPDOC_allstream_01Mar2021* <- this is the unaggregated version of the file above.
	* date: date of observation in YYYY-mm-dd format
	* stream_no: stream where data obtained (stream number from LSPA LMP files)
	* TP_mmolm3: total phosphorus in millimols per miter cubed (mean if TP_rep >= 2)
	* TN_mmolm3: total nitrogen in millimols per meter cubed (mean if TN_rep >= 2)
	* DOC_mmolm3: dissolved organic carbon in millimols per meter cubed	(mean if DOC_rep >= 2)
	* flag: flag pertinent to the column (TN, TP or DOC) with reported data; BDL = 'below detection limit'; suspect = observations should be used with caution
	* source: data provenence
	
# Lake
*chlorophylla_1986-2020_v01Mar2021* <- all chlorophyll data for years 1986-2020. Data provenence
	listed in datasource column, the primary sources are the LSPA's LMP files and the trophic
	reports provided by Woody. (replaces chlorophylla_1986-2019_v28Sept2020)
...
*lake_chem_1986-2020_v01March2021* <- any available chemistry/nutrient data for Sunapee for years
	1986-2020. Data provenece listed in datasource column, the primary sources are the LSPA's 
	LMP files and the trophic reports provided by Woody. (replaces lake_chem_1986-2019_v28Sept2020)
...
*dotemp_1986-2020_v01Mar2021* <- any available low-resolution dissolved oxygen or 
...
*secchi_1986-2020_v01Mar2021* <- this file lists secchi depths measured in Sunapee at all sites. Data
